...
export const WorkflowProgress: React.FC<WorkflowProgressProps> = ({
  ...
  return (
    <div
      id="workflow-progress-panel"
      className="bg-zinc-900/90 border border-zinc-800 rounded-xl p-4 shadow-xl mb-6 backdrop-blur-sm"
    >
      {/* Header bar */}
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-semibold text-white font-mono tracking-tight">
                {workflow.status === "running" &&
                  "Executing Durable Analysis Workflow..."}
                {workflow.status === "completed" &&
        ...
        <div className="flex items-center gap-3 self-end sm:self-auto">
          <div className="text-right hidden sm:block">
            <span className="text-xs font-mono text-zinc-400">
              {completedCount} of {workflow.steps.length} steps
            </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2.5 mt-3">
        {workflow.steps.map((step, idx) => {
          const isCurrent =
            workflow.currentStepIndex === idx && workflow.status === "running";
            ...
            >
              <div className="flex items-start gap-2">
                <div className="mt-0.5 flex-shrink-0">
                  ...
                  {isPending && (
                    <span className="w-3.5 h-3.5 inline-block rounded-full border border-zinc-700 text-center text-[9px] leading-3 text-zinc-600">
                      {idx + 1}
                    </span>
                  )}
                </div>
