        ...
        <div className="mt-3 pt-3 border-t border-zinc-800/60">
          <div className="flex items-center gap-2 text-xs text-zinc-400 mb-2">
            ...
            <span className="text-[11px] text-zinc-500">
              Click to start durable analysis
            </span>
          </div>
                ...
                {msg.checklist && (
                  <div className="mt-3 p-3.5 rounded-lg bg-zinc-950 border border-zinc-800 text-zinc-200 font-sans">
                    <div className="flex items-center justify-between mb-2 pb-2 border-b border-zinc-800">
                      ...
                      <span className="text-[10px] text-zinc-500 font-mono">
                        {msg.checklist.items.filter((i) => i.done).length} /{" "}
                        {msg.checklist.items.length} completed
                      </span>
                    </div>
                    ...
                    <div className="space-y-2">
                      {msg.checklist.items.map((item) => (
                        <button
                          ...
                          className="w-full text-left flex items-start gap-2.5 p-2 rounded-md hover:bg-zinc-900 cursor-pointer transition-colors group"
                        >
                          <span className="mt-0.5 text-zinc-400 group-hover:text-orange-400">
                            {item.done ? (
                          ...
                          <div className="flex-1 min-w-0">
                            <span
                              className={`text-xs ${item.done ? "line-through text-zinc-500" : "text-zinc-200"}`}
                            >
                              {item.task}
                            </span>
        ...
        {isReplying && (
          <div className="flex gap-3 justify-start">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5 text-zinc-400 text-xs flex items-center gap-2">
              ...
              <span>
                RepoPilot agent is reasoning over repository evidence...
              </span>
            </div>
          </div>
        )}
