...
import { StructuredReport, WorkflowExecution, ChatMessage, RepoComparison } from './types';

export default function App() {
  const [activeTab, setActiveTab] = useState<'chat' | 'report' | 'compare' | 'architecture'>('chat');
  ...
  const handleAnalyzeRepo = async (repoUrl: string) => {
    ...
    setWorkflow({
      ...
      steps: [
        { id: 'step-1', label: 'Fetch Metadata', description: 'Querying repository stats, stars, default branch...', status: 'running' },
        { id: 'step-2', label: 'Inspect Tree', description: 'Recursive repository file indexing...', status: 'pending' },
        { id: 'step-3', label: 'Extract Source', description: 'Targeting manifests and Docker compose files...', status: 'pending' },
        { id: 'step-4', label: 'Topological Review', description: 'Mapping component boundaries...', status: 'pending' },
        { id: 'step-5', label: 'LLM Reasoning', description: 'Evaluating evidence with strict constraints...', status: 'pending' },
        { id: 'step-6', label: 'Persist Report', description: 'Schema validation and commit to Durable Object...', status: 'pending' }
      ]
    });
    ...
    try {
      ...
      // Add to allReports if not already present
      setAllReports(prev => {
        const filtered = prev.filter(r => r.id !== data.report.id && r.repoName !== data.report.repoName);
      ...
      if (workflow) {
        setWorkflow(prev => prev ? { ...prev, status: 'failed', error: err.message } : null);
      }
    } finally {
    ...
    setMessages(prev => prev.map(m => {
      if (m.checklist && m.checklist.id === checklistId) {
        return {
          ...
          checklist: {
            ...m.checklist,
            items: m.checklist.items.map(item => item.id === itemId ? { ...item, done } : item)
          }
        };
      }
      return m;
  ...
  const handleTriggerCompare = async (repoAId: string, repoBId: string): Promise<RepoComparison | null> => {
    try {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col font-sans selection:bg-orange-500/30 selection:text-orange-200">
      ...
      <Header
        ...
        activeRepoName={currentReport ? `${currentReport.owner}/${currentReport.repoName}` : undefined}
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8">
        ...
        {errorBanner && (
          <div className="mb-4 p-3 bg-red-500/10 border border-red-500/30 rounded-xl text-xs text-red-300 flex items-center justify-between">
            ...
            <button onClick={() => setErrorBanner(null)} className="text-red-400 hover:text-red-200 font-bold ml-3">
              ✕
            </button>
          </div>
        )}
      ...
      <footer className="border-t border-zinc-900 bg-zinc-950 py-4 px-6 text-center text-xs text-zinc-500 font-mono">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
          <span>RepoPilot &copy; 2026 — Built on Cloudflare Agent and Workflow Primitives</span>
