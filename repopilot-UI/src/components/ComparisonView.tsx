...
import { StructuredReport, RepoComparison } from '../types';
import { ArrowLeftRight, GitCompare, Shield, CheckCircle, Cpu, Activity, Server, Sparkles, RefreshCw } from 'lucide-react';

interface ComparisonViewProps {
  reports: StructuredReport[];
  onTriggerCompare: (repoAId: string, repoBId: string) => Promise<RepoComparison | null>;
...
export const ComparisonView: React.FC<ComparisonViewProps> = ({
  ...
  const [comparison, setComparison] = useState<RepoComparison | null>(initialComparison || null);
  ...
  const getDimensionIcon = (name: string) => {
    switch (name.toLowerCase()) {
      case 'architecture pattern': return <Cpu className="w-4 h-4 text-orange-400" />;
      case 'data layer & caching': return <Server className="w-4 h-4 text-blue-400" />;
      case 'reliability & idempotency': return <CheckCircle className="w-4 h-4 text-emerald-400" />;
      case 'observability & telemetry': return <Activity className="w-4 h-4 text-purple-400" />;
      case 'security & secrets': return <Shield className="w-4 h-4 text-red-400" />;
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-zinc-900/90 border border-zinc-800 rounded-2xl p-6 shadow-xl backdrop-blur-sm">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-base font-bold text-white font-mono">Cross-Repository Architectural Comparison</h2>
            <p className="text-xs text-zinc-400 font-sans">
              Compare two evaluated repositories across Architecture, Reliability, Observability, Security, Scalability, and Deployment.
            </p>
          </div>
        </div>
        ...
        {reports.length < 2 ? (
          <div className="p-4 bg-zinc-950 rounded-xl border border-zinc-800 text-xs text-zinc-400">
            <p className="mb-2 text-zinc-300 font-semibold">
              Currently {reports.length} repository evaluated. You need at least two repositories to run side-by-side comparisons.
            </p>
            <p className="mb-3 text-zinc-400">
              Quickly analyze another repository to unlock full comparative analytics:
            </p>
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => onAnalyzeRepo('https://github.com/expressjs/express')}
              ...
              <button
                onClick={() => onAnalyzeRepo('https://github.com/fastapi/fastapi')}
                className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-white rounded-lg font-mono text-xs transition-colors"
              >
                + Analyze fastapi/fastapi
              </button>
              <button
                onClick={() => onAnalyzeRepo('https://github.com/supabase/supabase')}
                className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-white rounded-lg font-mono text-xs transition-colors"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
            <div>
              <label className="text-xs font-mono text-zinc-400 block mb-1.5">Repository A</label>
              <select
                id="select-repo-a"
                value={repoAId}
            ...
            <div>
              <label className="text-xs font-mono text-zinc-400 block mb-1.5">Repository B</label>
              <select
                id="select-repo-b"
                value={repoBId}
            ...
            <div className="sm:col-span-2 pt-2">
              <button
                ...
                disabled={loading || !repoAId || !repoBId || repoAId === repoBId}
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            {comparison.dimensions.map((dim, idx) => (
              <div key={idx} className="bg-zinc-900/70 border border-zinc-800 rounded-xl p-4 shadow-sm">
                <div className="flex items-center gap-2 mb-3 pb-2 border-b border-zinc-800/80">
                  {getDimensionIcon(dim.name)}
                  <h4 className="font-mono text-xs font-bold text-white">{dim.name}</h4>
                  <span className="ml-auto text-[10px] text-zinc-500 font-mono">Dimension {idx + 1} of 6</span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-sans">
                  <div className="p-3 bg-zinc-950 rounded-lg border border-zinc-800/80">
                    ...
                    <p className="text-zinc-300 leading-relaxed">{dim.repoAObservation}</p>
                  </div>
                  ...
                  <div className="p-3 bg-zinc-950 rounded-lg border border-zinc-800/80">
                    ...
                    <p className="text-zinc-300 leading-relaxed">{dim.repoBObservation}</p>
                  </div>
                </div>
                ...
                <div className="mt-3 pt-2 border-t border-zinc-800/60 text-xs flex items-center gap-2">
                  ...
                  <span className="text-emerald-400 font-medium font-mono text-[11px]">{dim.verdict}</span>
                </div>
              </div>
            ))}
          </div>
